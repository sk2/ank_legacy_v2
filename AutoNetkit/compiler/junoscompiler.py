        ...
        for node in self.network.q(platform="NETKIT"):
            ...
            for src, dst, data in self.network.graph.edges(node, data=True):
                ...
                interfaces.append({
                    ...
                    'description':  'Interface %s -> %s' % (self.network.label(src), 
                        self.network.label(dst)),
                })
#OSPF
...
#TODO: change this to use with for opening file
            f_jun = open( os.path.join(router_dir(self.network, node),
                                     "juniper.conf"), 'w')
